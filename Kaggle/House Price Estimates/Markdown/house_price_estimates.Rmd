...
types_mapping <- read_csv("/Users/Aaron/Documents/Data Science/learning_data_science/Kaggle/House Price Estimates/Inputs/identify_column_class_complete.csv")
```
...
#                       pull(class_type) %>%
#                       paste(collapse = ""))
hp_data <- read_csv("/Users/Aaron/Documents/Data Science/learning_data_science/Kaggle/House Price Estimates/Data/train.csv", 
                    col_types = types_mapping %>%
                      pull(class_type) %>%
                      paste(collapse = ""))
...
Now I can conduct the correlation analysis, and see which features have the largest effect on the class variable.
Once I have the results from the test, I can create a histgram to visualise any features with a high correlation to the class.
...
### Visualisation
I now have a list a features to look at and further determine their relationship with the class.
...
#### Overall Quality
```{r OverallQuality.8BoxPlot}
hp_training_data_dummy %>%
  select(SalePrice, OverallQual_8) %>%
  mutate(OverallQual_8 = factor(OverallQual_8, levels = 0:1)) %>%
  ggplot(aes(x = `OverallQual_8`, y = SalePrice, colour = `OverallQual_8`)) +
  geom_boxplot()
```
...
hp_training_data %>%
  ...
  mutate(OverallQual = factor(OverallQual, levels = 1:10)) %>%
  ggplot(aes(x = OverallQual, y = SalePrice, colour = OverallQual)) +
  geom_boxplot()
```
